...
public class BoardController {
	...
	/*
	 * @GetMapping("*") public ModelAndView getBoard() { ModelAndView mav = new
	 * ModelAndView(); Board board = new Board(); mav.addObject("board", board);
	 * return mav; }
	 */
	...
	@RequestMapping("list")
	public ModelAndView list(Integer pageNum, String searchtype, String searchcontent) {	//스프링에서는 모든 파라미터를 객체형태로 전달(int가 아니고 Integer이여야 함)
		ModelAndView mav = new ModelAndView();
		...
		int limit = 10;	//한페이지당 보여질 게시물의 건수
	...
	public ModelAndView detail(int num) {
		...
		service.updatercnt(num);
		mav.addObject("board", board);
		return mav;
	}
	
	@RequestMapping("imgupload")
	//MultipartFile upload : CKEditor 에서 전달해 주는 파일 정보의 이름
	//						 <input type="file" name="upload">
	//CKEditorFuncNum : CKEditor 내부에서 사용되는 파라미터
	//request : 업로드되는 파일의 위치를 지정하기 위함
	...
	/*
		...
		2. 답변글 데이터 추가 => service에서 처리
			- grp에 해당하는 레코드 grpstep 값 보다 큰 grpstep 의 값을 grpstep + 1
	...
	*/
	...
	public ModelAndView reply(@Valid Board board,BindingResult bresult, HttpServletRequest request) {
		...
		try {
			service.boardReply(board);	//board : 화면에서 전달받은 파라미터 정보 저장
			mav.setViewName("redirect:list.shop");
		} catch (Exception e) {
			e.printStackTrace();
			throw new BoardException("답변 작성시 오류 발생", "reply.shop?num="+board.getNum());
		}
		return mav;
	}
	...
	public ModelAndView update(@Valid Board board, BindingResult bresult, HttpServletRequest request) {
		...
		Board dbBoard = service.selectOne(board.getNum());
	...
	/*
	 * 1. num, pass 파라미터 저장
	 * 2. db의 비밀번호와 입력된 비밀번호가 틀리면 error.login.password 코드 입력. => 유효성 검증 내용 출력하기
	 * 3. db에서 해당 게시물 삭제
	...
	@PostMapping("delete")
	public ModelAndView delete(Board board, BindingResult bresult) {	//유효성검증을 하면안됨. 글로벌에러만 설정할것
		ModelAndView mav = new ModelAndView();
		try {
			if(!board.getPass().equals(service.selectOne(board.getNum()).getPass())) {
