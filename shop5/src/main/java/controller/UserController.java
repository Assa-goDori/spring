...
public class UserController {
	...
	public ModelAndView userEntryForm(@Valid User user, BindingResult bresult) throws Exception {
		ModelAndView mav = new ModelAndView();
		...
		try {
			//1.비밀번호는 해쉬알고리즘으로 값을 생성 하여 db에 저장
			//2. 이메일은 암호화(키는 userid 해쉬 값의 16자리)하여 db에 저장
	...
	public ModelAndView checkupdate(@Valid User user, BindingResult bresult, HttpSession session) {
		...
		try {
			...
			if(loginUser.getUserid().equals(user.getUserid())) {
				email = CipherUtil.decrypt(user.getEmail(), userid.substring(0, 16));
				user.setEmail(email);
				password = CipherUtil.makehash(user.getPassword());
				user.setPassword(password);
	...
	public ModelAndView checkdelete(String userid, String password, HttpSession session) {
		...
		try {
			hashpass = CipherUtil.makehash(password);
		} catch (Exception e) {
			e.printStackTrace();
		}
